...
import torch
from model import LSTMModel, RNNModel
from config import *
#sys.path.append("../")
from utils import sequences_collator, get_acc
...
for load_from_pt in avail_ckpts:
    ...
    for split in args.test_files.split():
        ...
        msg = f""" {split} acc, load from {load_from_pt}
                ...
                | Test Acc: {round(last_correct/last_demo, 4)}
            """
        messages.append(msg)
        print(msg)
        ...
        json.dump({
                ...
                "test_acc": round(last_correct/last_demo, 4),
...
with open(f"{config.output_dir}/{args.handle}/terminal_tester{args.suffix}.txt", "w") as f:
